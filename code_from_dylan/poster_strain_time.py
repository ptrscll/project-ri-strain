...
if k == 2:
    ...
    for i in range(0, last_mesh_num + 1):
        if i in [0, 3, 10]:
            ...
            vp.plot2D(file, field='comp_field', cfields=['initial crust_upper','initial crust_lower','initial mantle_lithosphere'], bounds=bounds,ax=axs[j][0], cmap='tab20c')
            ...
            axs[j][2].set_ylabel('Normalized Noninitial Strain')
            axs[j][2].set_xlabel('x (km)')
            ...
            for i in range(0, len(valid_indices)):
                if valid_indices[i]:
                    ...
                    if prev == True:
                        axs[j][1].axvspan(start, 300 + i, facecolor='blue', alpha=0.25)
                        axs[j][2].axvspan(start, 300 + i, facecolor='blue', alpha=0.25)

                    prev = False
            if prev == True:
                axs[j][1].axvspan(start, 700, facecolor='blue', alpha=0.25)
                axs[j][2].axvspan(start, 700, facecolor='blue', alpha=0.25)
    ...
    for j in range(0, 3):
        ...
        axs[j][2].set_ylim(norm_y_min, 1.0)
